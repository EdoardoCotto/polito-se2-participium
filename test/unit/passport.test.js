...
const setup = () => {
  ...
  jest.isolateModules(() => {
    ...
    const passportMock = {
      _lastStrategy: null,
      _lastSerialize: null,
      _lastDeserialize: null,
      use: jest.fn((strategy) => { passportMock._lastStrategy = strategy; }),
...
describe('utils/passport.js', () => {
  ...
  test('serializeUser stores user.id', () => {
    const { passport } = setup();
    const serialize = passport._lastSerialize;
    const done = jest.fn();
  ...
  test('deserializeUser resolves user', async () => {
    ...
    const deserialize = passport._lastDeserialize;
